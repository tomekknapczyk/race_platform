                                    ...
                                    <h5 class="m-0 col-3 text-right">
                                        ...
                                        <div class="btn-group">
                                            <button class="btn btn-sm btn-info editSign" data-toggle="modal" data-target="#editSign" data-id="{{ $sign->id }}"
                                                data-text=
                                                '{"id":"{{ $sign->id }}", "marka":"{{ $sign->marka }}", "model":"{{ $sign->model }}", "rok":"{{ $sign->rok }}", "ccm":"{{ $sign->ccm }}", "nr_rej":"{{ $sign->nr_rej }}", "driver_name":{!! json_encode($sign->name) !!}, "driver_lastname":{!! json_encode($sign->lastname) !!}, "driver_address":{!! json_encode($sign->address) !!}, "driver_id_card":"{{ $sign->id_card }}", "driver_phone":"{{ $sign->phone }}", "driver_email":"{{ $sign->email }}", "driver_driving_license":"{{ $sign->driving_license }}", "car_oc":{!! json_encode($sign->oc) !!}, "car_nw":{!! json_encode($sign->nw) !!}, "pilot_name":{!! json_encode($sign->pilot_name) !!}, "pilot_lastname":{!! json_encode($sign->pilot_lastname) !!}, "pilot_address":{!! json_encode($sign->pilot_address) !!}, "pilot_id_card":"{{ $sign->pilot_id_card }}", "pilot_phone":"{{ $sign->pilot_phone }}", "pilot_email":"{{ $sign->pilot_email }}", "pilot_driving_license":"{{ $sign->pilot_driving_license }}", "klasa":"{{ $sign->klasa }}", "advance":"{{ $sign->advance }}"}'
