                                    ...
                                    <h5 class="m-0 col-3 text-right">
                                        ...
                                        <div class="btn-group">
                                            <button class="btn btn-sm btn-info editSign" data-toggle="modal" data-target="#editSign" data-id="{{ $sign->id }}"
                                                data-text=
                                                '{"id":"{{ $sign->id }}", "marka":"{{ $sign->marka }}", "model":"{{ $sign->model }}", "rok":"{{ $sign->rok }}", "ccm":"{{ $sign->ccm }}", "nr_rej":"{{ $sign->nr_rej }}", "driver_name":"{{ $sign->name }}", "driver_lastname":"{{ $sign->lastname }}", "driver_address":"{{ $sign->address }}", "driver_id_card":"{{ $sign->id_card }}", "driver_phone":"{{ $sign->phone }}", "driver_email":"{{ $sign->email }}", "driver_driving_license":"{{ $sign->driving_license }}", "car_oc":"{{ $sign->oc }}", "car_nw":"{{ $sign->nw }}", "pilot_name":"{{ $sign->pilot_name }}", "pilot_lastname":"{{ $sign->pilot_lastname }}", "pilot_address":"{{ $sign->pilot_address }}", "pilot_id_card":"{{ $sign->pilot_id_card }}", "pilot_phone":"{{ $sign->pilot_phone }}", "pilot_email":"{{ $sign->pilot_email }}", "pilot_driving_license":"{{ $sign->pilot_driving_license }}", "pilot_oc":"{{ $sign->pilot_oc }}", "pilot_nw":"{{ $sign->pilot_nw }}", "klasa":"{{ $sign->klasa }}", "advance":"{{ $sign->advance }}"}'
